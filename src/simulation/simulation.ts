...
function handleRandomPlantGenerator(plantsSubject: BehaviorSubject<Plant[]>): Observable<void> {
    ...
    return timer(0).pipe(
        ...
        switchMap(() => handleRandomPlantGenerator(plantsSubject))
    )
}
...
export default function startSimulation(plantsCount: number, rabbitsCount: number, foxesCount: number, simulationSpeedSubject: Observable<number>) {

    const frame$ = simulationSpeedSubject.pipe(
    ...
    const randomPlantGeneratorSubscription = handleRandomPlantGenerator(plantsSubject).subscribe()
