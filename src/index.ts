...
getButtonInputStream('startBtn').pipe(
    ...
    tap(([_, plantsCount, rabbitsCount, foxesCount]) => {
        ...
        startSimulation(plantsCount, rabbitsCount, foxesCount, simulationSpeedSubject)
    })
).subscribe()

getButtonInputStream('stopBtn').pipe(
    tap(() => SimulationSubscriptions.clearSubscriptions())
).subscribe()
